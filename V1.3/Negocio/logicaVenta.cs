class logicaVenta
{
    ...
    private LogicaPromo logicaPromo;
 
    public logicaVenta(Inventario inventario, Ventas ventas, LogicaPromo logicaPromo)
    {
        this.inventario = inventario;
        this.ventas = ventas;
        this.carrito = new Carrito();
        this.logicaPromo = logicaPromo;
    }
    ...
    public Carrito? realizarVenta(Usuario usuario)
    {
        if (!carritoValido()) return null;

        double descuentoTotal = obtenerDescuentoTotal(usuario.getRol());
        carrito.setDescuento(descuentoTotal);
    ...
    //Promociones
    public double obtenerDescuento(Rol rol, int posCarrito)
    {
        double montoTotal = carrito.obtenerProducto(posCarrito)?.getTotal() ?? 0.0;
        return logicaPromo.obtenerDescuento(montoTotal, rol);
    }

    public double obtenerDescuentoTotal(Rol rol)
    {
        double montoTotal = carrito.getTotal();
        double descuento = 0.0;
        foreach(itemCarrito item in carrito.GetCarrito() ?? new List<itemCarrito>())
        {
            descuento += logicaPromo.obtenerDescuento(item.getTotal(), rol);
        }
        return descuento;
    }

    public LogicaPromo getLogicaPromo() { return logicaPromo; }
}